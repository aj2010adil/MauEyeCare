# MauEyeCare Main App - With Camera Integration
...
def main():
    ...
    with tab4:
        ...
        st.subheader("📸 AI Face Analysis & Spectacle Recommendation")
        
        # Camera capture option
        col1, col2 = st.columns(2)
        
        with col1:
            st.markdown("**Option 1: Capture Live Photo**")
            if st.button("📷 Open Camera & Capture", type="primary"):
                try:
                    ...
                    if image is not None:
                        ...
                        st.info("• Ensure camera is connected")
                        st.info("• Try uploading photo instead")
        ...
        with col2:
            ...
            uploaded_file = st.file_uploader("Upload patient photo", type=['jpg', 'jpeg', 'png'])
        ...
        # Analysis section
        if 'camera_image' in st.session_state and 'patient_name' in st.session_state:
            st.markdown("---")
            ...
            with col1:
                if st.button("🔍 Basic Face Analysis"):
                    try:
                        face_shape, recommendations = get_face_shape_basic(st.session_state['camera_image'])
                ...
                if st.button("🤖 AI-Powered Analysis", type="primary"):
                    try:
                        with st.spinner("AI analyzing face and generating recommendations..."):
                            ai_analysis = analyze_face_with_ai(
                            ...
                            st.session_state['ai_face_analysis'] = ai_analysis
                            
                            # Create shopping list
                            shopping_items = create_spectacle_shopping_list(ai_analysis)
                            st.session_state['spectacle_shopping_list'] = shopping_items
            ...
            with col2:
                ...
                if 'face_analysis' in st.session_state:
                    analysis = st.session_state['face_analysis']
                    st.success(f"**Face Shape:** {analysis['shape']}")
                ...
                # Display AI analysis
                if 'ai_face_analysis' in st.session_state:
                    st.markdown("**🤖 AI Analysis:**")
                    
                    with st.expander("View Full AI Recommendations", expanded=True):
                        st.write(st.session_state['ai_face_analysis'])
                    
                    # Shopping list
                    if 'spectacle_shopping_list' in st.session_state:
                        st.markdown("**🛒 Recommended Items:**")
                        
                        items = st.session_state['spectacle_shopping_list']
                        for item in items:
                            col_item, col_add = st.columns([3, 1])
                            with col_item:
                                st.write(f"• {item}")
                            with col_add:
                                if st.button("➕", key=f"add_item_{item}", help=f"Add {item}"):
                        ...
                        if st.button("🛒 Add All to Inventory", type="primary"):
                            for item in items:
                                add_or_update_inventory(item, 5)
                            st.success(f"Added {len(items)} items to inventory!")
                            st.balloons()
        
        elif 'camera_image' in st.session_state:
            st.info("Please select a patient first to get personalized recommendations.")
        else:
            st.info("Capture or upload a patient photo to begin analysis.")
